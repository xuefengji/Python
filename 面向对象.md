...
+ 定制类
  ...
  + ```__iter__```： 如果一个类想被用于`for ... in`循环，类似list或tuple那样，就必须实现一个`__iter__()`方法 ， 该方法返回一个迭代对象，然后，Python的for循环就会不断调用该迭代对象的`__next__()`方法拿到循环的下一个值，直到遇到`StopIteration`错误时退出循环 
  ...
  + ```__getsttr__```：当调用不存在的属性时， Python解释器会试图调用`__getattr__` 获取，可以自定义返回字符串，不会产生出错信息
